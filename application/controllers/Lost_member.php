...
class Lost_member extends CI_Controller {
	...
	public function index(){
		...
		if ($this->input->post('submit', TRUE) == 'Submit') 
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				$get = $this->m_admin->get_where('t_member', array('email' => 
				...
				if ($get->num_rows() > 0) 
				{
					...
					$this->email->message('
						...
						<a href="'.base_url().'lost_member/reset/'.$key.'">Disini</a>
					...
					if ($this->email->send())
					{
						...
						$this->m_admin->update('t_member', $data, $cond);
		...
		$this->load->view('member/lost_member');
	}

	function reset()
	{
		if ($this->uri->segment(3)) {
			$this->load->view('member/form_reset');
